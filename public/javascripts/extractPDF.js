      ...
      keywords.map(word => {
        ...
        for (let i = 0; i < pages.length; i++) {
          ...
          if (pages[i].toLowerCase().indexOf(keyword.toLocaleLowerCase()) !== -1) {
            ...
            var pdfWriter = hummus.createWriter(path.join(outputFolder, `${name}.pdf`));
...
function modify() {
  ...
  for (let single of source) {
    ...
    let modified = __dirname + `/modified/${single}-modified.pdf`
    ...
    keyword.forEach(word => {
      text = text.replace(new RegExp(word, 'g'), "**" + word.toUpperCase() + "**")
    })
    doc.pipe(fs.createWriteStream(modified))
    doc.text(text)
    doc.end()
  }
}
